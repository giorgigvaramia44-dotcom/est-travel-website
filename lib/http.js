...
function resolveStaticPath(urlPath) {
  ...
  if (absolute !== path.resolve(publicDir) && !absolute.startsWith(publicRoot)) return null;
  return absolute;
}
...
function requestedCacheControl(filePath) {
  if (filePath.includes(`${path.sep}uploads${path.sep}`)) return 'public, max-age=3600';
